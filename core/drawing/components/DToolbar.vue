...
>
    <!-- {{ tool }} -->
      ...
      :class="(state.currentTool === tool.id || preview  === tool.id) && '!border-gray-500 bg-gray-100'"
    ...
    <template v-if="tool.children && preview  === tool.id ">
      <button v-for="(child, index) in tool.children"        @click="handleToolClick(child.id, tool.id)"
 class="border border-transparent rounded absolute left-10" :style="{ top: `${index * 35}px`, }"
...
// const handleChildClick = (parentToolId: string, childId: string) {
//     tools._setCurrent(childId);
//   emit('select', childId);
