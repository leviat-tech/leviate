...
import { convertFromSI } from '@/extensions/units';
...
yup.addMethod(yup.mixed, 'oneOfCoerced', function oneOfCoerced(options, _default) {
  return this.oneOf(options).transform((value) => (!options.includes(value) ? _default || options[0] : value));
});
...
yup.addMethod(yup.number, 'minConverted', function minConverted(value, precision) {
  const unit = this.describe().meta?.units;
  let val = unit ? convertFromSI(value, unit) : value;
  val = (precision != null) ? parseFloat(val.toFixed(precision), 10) : val;
  const message = `must be greater than or equal to ${val}${unit}`;
  return this.min(value, message);
});

yup.addMethod(yup.number, 'maxConverted', function maxConverted(value, precision) {
  const unit = this.describe().meta?.units || '';
  let val = unit ? convertFromSI(value, unit) : value;
  val = (precision != null) ? parseFloat(val.toFixed(precision), 10) : val;
