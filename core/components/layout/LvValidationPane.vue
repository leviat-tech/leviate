<template>
  <div class="text-xs">
    <div class="bg-base-50 h-12 w-full items-center border-t border-base-300 px-2 grid grid-cols-10">
      ...
      <div class="cursor-pointer text-base-600 hover:text-base-800 col-span-2 flex justify-center" @click="togglePanel">
        <ChevronDownIcon class="h-6 w-6" v-if="isExpanded" />
        <ChevronUpIcon class="h-6 w-6" v-else />
      </div>
      <div class="col-span-4 flex justify-end">
        <div class="flex space-x-2 cursor-pointer hover:text-base-800 text-base-600 pr-0.5" v-if="isExpanded">
          <ArrowsPointingOutIcon class="h-6 w-6 p-0.5" v-if="hasModal" @click="openModal" />
          <FunnelIcon class="h-6 w-6 p-0.5" :class="[filtersClass]" @click="filtersActive = !filtersActive"  />
        </div>
    ...
    <div class="h-48 flex " v-if="isExpanded">
      <div class="border-t border-base-300 w-full p-2 space-y-2 overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-base-100 [&::-webkit-scrollbar-thumb]:bg-base-400">
        <CStatusMessage v-for="message in displayedMessages" :status="message.type" :content="message.content" :timestamp="message.timestamp">
          <CIcon v-if="message.isDismissable === true" type="plus" class="w-4 flex-none rotate-45 justify-end cursor-pointer" @click="$emit('dismiss', message.id)"/>
        </CStatusMessage>
        <div class="justify-between flex py-0.5 font-medium">
          <div class="">{{ $L('displaying') }} {{ displayedMessages.length }} {{ $l('of') }} {{ messages.length }} {{ $l('messages') }}</div>
          <div class="text-brand-700 hover:text-brand-900 cursor-pointer" v-if="messages.length > displayCount" @click="loadMore">{{ $L('load_more') }}</div>
        </div>
      </div>
      <div class="w-11 px-2 py-2 flex flex-col justify-between transition-all ease-in-out duration-150 bg-base-50 border-l border-base-300" v-if="filtersActive">
        <CCheckbox no-label :color="color" v-model="filters[color]" class="-ml-1.5" colorUnchecked v-for="value, color in filters" :key="color" />
      </div>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  defaultMessageCount: { type: Number, default: 3 }
});
...
const openModal = () => { console.log('Not yet implemented') };
