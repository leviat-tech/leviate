<template>
  <div
    :class="{ 'flex-1 flex flex-col': isActive }"
    class="bg-base-50 transition-all duration-300 ease-in"
  >
    <div
      @click="onClick(name)"
      ...
      :class="{
        'bg-white hover:bg-base-50': !isActive,
        'bg-[#FAFCFC] text-black font-semibold border-t-2 border-brand-500': isActive,
      }"
    >
    ...
    <div v-if="isActive" class="bg-[#FAFCFC] flex-1 relative">
      <div
        class="h-full overflow-y-auto absolute w-full text-sm py-2 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-base-100 [&::-webkit-scrollbar-thumb]:bg-base-400"
      >
        <slot />
      </div>
    </div>
