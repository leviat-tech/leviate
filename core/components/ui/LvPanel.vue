<template>
  <div
    class="bg-white h-full flex flex-col justify-between overflow-hidden"
    :data-cy="`panel__${panelId}`"
    :class="{ 'flex-none': isExpanded }"
    :style="`width:${widthValue}px`"
  >
    <!-- Panel content -->
    <div class="flex-1 overflow-hidden">
      <div class="h-full flex flex-col" :class="{ 'flex-none': isExpanded }" v-if="isExpanded">
        <slot />
      </div>
      ...
      >
        <div
          :class="{
            'text-xs font-semibold [writing-mode:vertical-lr] whitespace-nowrap rotate-180':
              !isExpanded,
          }"
        >
          {{ $L(panelId) }}
    ...
    <div
      v-if="!disabled"
      class="w-full bg-base-50 flex items-center border-t border-base-300 h-12"
      :class="isExpanded && !flip ? 'justify-end' : 'justify-start'"
    >
      <button
        @click="store.setPanelIsExpanded(panelId, !isExpanded)"
        class="flex justify-center items-center w-[59px] h-10 text-base-600 hover:text-base-800 outline-none"
      >
